...
export default function Login() {
  ...
  const login = async (event) => {
    ...
    try {
      ...
      setErrorMessage(error.response?.data?.error ?? 'No pudimos iniciar sesion. Verifica email, contrasena y que la API este activa.');
    }
  };
    ...
    <div style={{ background: 'linear-gradient(135deg, #100608 0%, #070204 100%)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.07)', backdropFilter: 'blur(20px)', borderRadius: 24, padding: 40, width: '100%', maxWidth: 400, boxShadow: '0 24px 64px rgba(0,0,0,0.6)', position: 'relative', zIndex: 10 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", color: '#f5edf0', fontSize: 32, margin: '0 0 6px', fontWeight: 700 }}>Hola de nuevo</h1>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, margin: 0, letterSpacing: '0.02em' }}>Ingresa a tu espacio Estetica Romi Paiva</p>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 14, fontSize: 13 }}>
          <Link to="/olvide-contrasena" style={{ color: 'rgba(255,255,255,.58)', textDecoration: 'none', fontWeight: 500 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '24px 0', gap: 10 }}>
          ...
          <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em' }}>O continuar con</span>
          <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
          <GoogleLogin
            ...
            onError={() => setErrorMessage('No se pudo iniciar sesion con Google. Proba con email y contrasena.')}
            theme="filled_dark"
            shape="pill"
            locale="es"
          />
        ...
        <div style={{ textAlign: 'center', marginTop: 28, fontSize: 13 }}>
          <span style={{ color: 'rgba(255,255,255,0.4)' }}>No tenes cuenta? </span>
